...
def test_two_builds_are_byte_identical(tmp_path):
    ...
    ports_path.write_text(json.dumps({"type":"FeatureCollection","features":[
        {"type":"Feature","properties":{"id":1,"name":"Fixture harbour","slips":12},"geometry":{"type":"Point","coordinates":[14.1,40.7]}},
        {"type":"Feature","properties":{"id":2,"name":"Outside"},"geometry":{"type":"Point","coordinates":[15.0,42.0]}}]}))
    config={
        ...
        "classification":{"version":"test-v1"},"ports_resource":"ports.json","ports_release":"fixture-ports-v1"}
    ...
    with DataTiles(work/"gaeta-to-maratea.datatiles",read_only=True) as store:
        profile=next(item for item in store.content_profiles() if item["coordinates"].get("variable")=="ports")
        assert profile["data_type"]=="vector" and profile["schema"]["featureCount"]==1
