...
The contour resource accepts a positive `interval` and optional `adaptive=true`. Adaptive mode uses the interval through 50 m depth, at least twice that spacing through 200 m, and at least ten times that spacing in deeper water, with floors of 10 m and 50 m respectively. Responses label contours as `minor`, `index`, or `major` and declare the applied spacing rule. This is a portrayal-oriented derivation over stored values, not additional bathymetric information.
